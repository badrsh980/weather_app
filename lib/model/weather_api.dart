...
class WeatherApi {
  ...
  Future<Weather> postNewWeather(Weather newWeather) async {
    final response = await http.post(
      Uri.parse("https://api.weatherapi.com/v1/current.json?key=$apiKey"),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(newWeather.toJson()),
    );
    if (response.statusCode == 200) {
      return Weather.fromJson(json.decode(response.body));
    } else {
      throw Exception('Failed to create weather object');
    }
  }
}
